...
def find_comps_all_local(infile,cind):
    # initializations
    sttime = time.time()
...
#    DL, ID, SpC, Event = cu.read_and_parse_input(infile)
    ...
    return [outVAL,notcomp,eltime, ip, c_iter]
    
    



def find_comps(SpC,Event,ID,DL,outIndex,outVAL):
    # initializations
    sttime = time.time()
    max_iter = 15  # maximum allowable iterations
    c_iter = 0     # iteration counter starting at zero
    ...
    while (c_iter <= max_iter) and (converged == False):
        ...
        if compcount[-1]-compcount[-2] == 0:
            converged = True
        c_iter += 1
    notcomp = ID[np.nonzero(COMPARABLE-1)[0]]
    eltime = time.time() - sttime
    return [outVAL,notcomp,eltime]
