...
all_out_inds = pickle.load(ifp)
ifp.close()

dropind_row = int(sys.argv[1])
dropinds = all_out_inds[dropind_row,:]
...
# find the set of the intentionally left-out IDs combined with not_comp_IDs
all_missing_IDs = np.hstack((notcomp_IDs,knock_out_IDs))
cHg,obsHg,cID = One_Sample_Hg(all_missing_IDs,infile)

outfile = 'Hg_%d.dat' %(dropind_row)
ofp = open(outfile,'w')
ofp.write('%d: row knocked out from all_out_inds.pkl\n' %(dropind_row))
ofp.write('ID calc_Hg obs_Hg\n')
for ii,cv in enumerate(cID):
    ofp.write('%d %f %f\n' %(cv,cHg[ii],obsHg[ii]))
ofp.close()
