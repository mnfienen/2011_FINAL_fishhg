...
allID = np.loadtxt('IDs.csv',skiprows=1)

dropind = int(sys.argv[1])
knock_out_ID = allID[dropind]
# initializations
...
outfile = 'comps_%d.dat' %(dropind)
ofp = open(outfile,'w')
ofp.write('%d ' %(knock_out_ID))
for cv in notcomp:
    ofp.write('%d ' %(cv))
ofp.write('\n')
ofp.close()
